<template>
  <v-container>
    <v-layout row wrap>
      <v-flex xs12>
        <h1>Dollhouse Miniatures Magazine</h1>
        <v-layout>
          <v-flex xs12 class="text-xs-center">
            <v-progress-circular indeterminate color="primary" v-if="loading" :size="50"></v-progress-circular>
          </v-flex>
        </v-layout>
        <v-card v-for="magazine in magazines" v-if="magazine.magcode === 'DHM'" :key="magazine.id">
          <v-container fluid>
            <v-layout row>
              <v-flex xs3 sm2 md1>
                <v-card-media
              ...
              <v-flex xs6 sm8 md9>
                <v-card-title primary-title class="pt-0 pb-0">
                  <div>
                    ...
                    <div><strong>Magcode: {{ magazine.magcode }} - {{ magazine.Magazine }}</strong></div>
                  </div>
                </v-card-title>
              </v-flex>
...
export default {
  computed: {
    magazines() {
      return this.$store.getters.loadedMagazines;
    },
    loading() {
